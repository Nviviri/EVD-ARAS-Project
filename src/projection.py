...
def get_block_screen_coordinate(layer, block_pos):
    ...
    screen_offset_x = (PROJECTOR_RESOLUTION[0] /
                       2) * (baseplate_real_size[0] / PROJECTION_SIZE[0]) \
        / ((CAMERA_HEIGHT - layer * BLOCK_HEIGHT) / CAMERA_HEIGHT)
    screen_offset_y = (PROJECTOR_RESOLUTION[1] /
                       2) * (baseplate_real_size[1] / PROJECTION_SIZE[1]) \
        / ((CAMERA_HEIGHT - layer * BLOCK_HEIGHT) / CAMERA_HEIGHT)
    baseplate_layer_top_left_screen_coords = (
        (PROJECTOR_RESOLUTION[0] / 2) - screen_offset_x,
        (PROJECTOR_RESOLUTION[1] / 2) - screen_offset_y
    )
