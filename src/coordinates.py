...
def calculate_nub_coordinate_matrix(baseplate_corners_pos, baseplate_corners_size):
    matrix = np.zeros((MAX_BLOCK_LAYERS, BASEPLATE_SIZE[1], BASEPLATE_SIZE[0], 2), dtype=np.float32)
    baseplate_midpoint = (
        ...
        for j in range(BASEPLATE_SIZE[1]):
            for i in range(BASEPLATE_SIZE[0]):
                matrix[k][j][i] = np.array([
                    baseplate_midpoint[0] + (i - (BASEPLATE_SIZE[0] / 2) + 0.5) * (baseplate_corners_size[0] / (BASEPLATE_SIZE[0] - 1)) / ((CAMERA_HEIGHT - k * BLOCK_HEIGHT) / CAMERA_HEIGHT),
                    baseplate_midpoint[1] + (j - (BASEPLATE_SIZE[0] / 2) + 0.5) * (baseplate_corners_size[1] / (BASEPLATE_SIZE[1] - 1)) / ((CAMERA_HEIGHT - k * BLOCK_HEIGHT) / CAMERA_HEIGHT)
                ], dtype=np.float32)
