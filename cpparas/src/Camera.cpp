...
void Camera::Camera_thread_worker()
{
    ...
    if (fpipe == NULL) {
        ...
        while (threadRunning) {
            ...
            // TODO: handle the case where readBytes != bufferSize
            (void)readBytes;
...
void Camera::Camera_calibrate()
{
    ...
    std::string cmd = "raspivid -md 2 --width " + std::to_string(width) + " --height " + std::to_string(height) + " --nopreview --ISO 100 --settings -o /dev/null -t 1";
    FILE* fpipe = popen(cmd.c_str(), "r");
    if (fpipe == NULL) {
        std::cout << "Failed to open camera / pipe / raspivid" << std::endl;
    } else {
